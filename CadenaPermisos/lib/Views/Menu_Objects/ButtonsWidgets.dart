...
class ButtonsWidgets extends StatefulWidget {
 ...
 final Menu_Functions menu_functions = Menu_Functions();
  ...
  Widget ConfimButton(String DocId, SharedPreferences preferences) {

    int? level = preferences?.getInt("Level_User");
    if (level! >= 2) {
      return Container(
        padding: const EdgeInsets.only(left: 0, right: 0, top: 10, bottom: 15),
        child: ElevatedButton.icon(
          onPressed: (){
            menu_functions.ConfimAuthorization(DocId, preferences);
          },
          icon: const Icon(Icons.check),
          label: const Text("Confim Validation",
              style: TextStyle(
